...
export class CppMLEngine {
  ...
  static async recordExecution(
    ...
    try {
      ...
      // const megaInsert = supabase.from("ml_execution_features").insert({ // Removed Supabase usage
      ...
      // const legacyInsert = supabase.from("ml_feature_vectors").insert({ // Removed Supabase usage
        execution_id: executionId,
        user_id: userId,
        feature_vector: featureVector,
        num_qubits: features.qubits,
