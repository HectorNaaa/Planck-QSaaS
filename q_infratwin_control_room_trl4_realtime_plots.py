...
# Q-InfraTwin TRL4 — Control Room (Streamlit 1.53 compatible) + Real-time plot evolution
# Run:
#   python -m pip install -r requirements_dashboard_q_infratwin_trl4.txt
#   python -m streamlit run q_infratwin_control_room_trl4_realtime_plots.py
#
...
st.markdown("""
This Control Room executes the TRL4 hybrid engine **continuously** and visualises the **evolution of KPIs and plots in real time**.
- ▶ Start / ⏸ Pause / ⏭ Step / ⏹ Reset
- Speed: Real-time (interval) or Max speed (batch)
- Target steps + Auto-stop
- Rolling plot window (last N steps)
""")

# -----------------------------
# Rerun helper (do NOT swallow Streamlit rerun exception)
# -----------------------------
def do_rerun():
...
ss.setdefault("interval_ms", 500)      # per step (real-time)
ss.setdefault("batch_steps", 25)       # per tick (max speed)
...
ss.setdefault("ui_refresh_ms", 200)    # rerun cadence while running
ss.setdefault("plot_window", 300)      # last N points to plot
...
def run_some_steps():
    """Run steps according to speed mode, respecting target_steps when auto_stop is on."""
...
with st.sidebar:
    ...
    ss.plot_window = st.slider("Plot window (last N steps)", 50, 10000, int(ss.plot_window), step=50)
...
# -----------------------------
# Continuous loop (sleep + rerun)
# -----------------------------
if ss.running:
    now = time.time()
    last = float(ss.last_step_ts)
...
if not ss.engine_loaded:
    st.info("Load engine from the sidebar.")
    st.stop()
...
# Rolling window for plots (real-time evolution)
w = int(ss.plot_window)
df_live = df.tail(w).copy()
df_live = df_live.reset_index(drop=True)
df_live["step"] = range(ss.step_id - len(df_live) + 1, ss.step_id + 1)

left, right = st.columns([2, 1])

with left:
    st.subheader("Real-time evolution (rolling window)")
    # These charts redraw every rerun, showing evolution live.
    st.line_chart(df_live.set_index("step")[["objective_value"]], height=220)
    st.line_chart(df_live.set_index("step")[["exec_ms"]], height=220)

    # Optional: show queue evolution if present
    if "qpu_queue_ms" in df_live and df_live["qpu_queue_ms"].notna().any():
        st.line_chart(df_live.set_index("step")[["qpu_queue_ms"]], height=220)

with right:
    st.subheader("Routing distribution (cumulative)")
    st.bar_chart(df["route"].value_counts())

    st.subheader("Fallback reasons (cumulative, top)")
    reasons = []
...
# Per-twin drilldown (also rolling window)
st.subheader("Per-twin drill-down (rolling window)")
twin_ids = sorted(df["twin_id"].unique().tolist())
sel = st.selectbox("Select twin_id", twin_ids, index=0)

dft = df[df["twin_id"] == sel].tail(w).copy()
dft = dft.reset_index(drop=True)
dft["step"] = range(max(1, ss.step_id - len(dft) + 1), ss.step_id + 1)
