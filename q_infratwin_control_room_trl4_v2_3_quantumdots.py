...
# Q-InfraTwin TRL4 — Control Room (Streamlit 1.53 compatible, no st.autorefresh)
# Run:
#   python -m pip install -r requirements_dashboard_q_infratwin_trl4.txt
#   python -m streamlit run q_infratwin_control_room_trl4_v2_2.py
#
...
Compatibility: **Streamlit 1.53** (no `st.autorefresh`).
""")

# -----------------------------
# Rerun helper (compatible)
# -----------------------------

def do_rerun():
    # IMPORTANT: do NOT swallow Streamlit's internal rerun exception.
    # Prefer experimental_rerun for broader compatibility.
...
ss.setdefault("plot_window", 300)

# UI refresh loop (how often we rerun while running)
...
# -----------------------------
if ss.running:
    # Enforce step cadence for real-time mode
    now = time.time()
    ...
    # Throttle UI refresh, then rerun
