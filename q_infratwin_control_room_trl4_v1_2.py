...
df = df_current()
if df.empty:
    # Show control-room status even before first step is executed
    s1, s2, s3, s4 = st.columns(4)
    s1.metric("Steps", str(st.session_state.get("step_id", 0)))
    s2.metric("Running", "Yes" if st.session_state.get("running") else "No")
    s3.metric("Mode", str(st.session_state.get("speed_mode")))
    s4.metric("Twins", str(st.session_state.get("twins")))
    st.info("Ready. Use the sidebar: **Load engine** → **Reset** → **Start** (or **Single step**).")
    st.stop()
...
def metric_row():
    ...
    c1.metric("Steps", str(st.session_state.get("step_id", len(df))))
